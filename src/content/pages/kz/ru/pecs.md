---
title: "PECS: система обмена изображениями для коммуникации"
summary: "Что такое PECS, чем он отличается от AAC в целом, что показывают исследования, как оценивать результат и когда рассматривать другие варианты."
description: "Практическое руководство для родителей о PECS при аутизме: протокол, доказательства, отличие от AAC, влияние на речь, перенос навыка, ограничения и выбор специалиста."
country: kz
language: ru
section: parents
topic: approaches-to-support
slug: pecs
translationKey: parents-pecs
translationStatus: source
audience:
  - parents
  - families
author: Ruslan Kurmashev
editor: Ruslan Kurmashev
riskLevel: lower
disclaimerType: informational
conflictOfInterest: "Конфликт интересов не заявлен."
publishedAt: 2026-09-18
updatedAt: "2026-09-20T15:05:25.596Z"
reviewDueAt: 2027-03-18
draft: false
keyPoints:
  - "PECS - это один конкретный подход AAC, а не синоним всей альтернативной и дополнительной коммуникации."
  - "Данные поддерживают некоторые коммуникационные результаты, особенно инициирование общения и просьбы, но не гарантируют развитие устной речи."
  - "PECS не является обязательной ступенью перед высокотехнологичной AAC."
  - "Результат важно оценивать по самостоятельной функциональной коммуникации и переносу навыка, а не по номеру освоенной фазы."
evidenceLimitations: "Доказательная база PECS неоднородна. Ранние исследования и обзоры показывали ограниченные или умеренные улучшения коммуникации и слабые результаты по устной речи. Метаанализ 2026 года из материкового Китая сообщил крупный объединённый эффект для коммуникации, но переносимость этих результатов на другие страны и условия требует осторожности. Сохранение навыка, его перенос и более широкие функции коммуникации изучены менее последовательно."
sources:
  - title: "Augmentative and Alternative Communication (AAC)"
    url: "https://www.asha.org/Practice-Portal/Professional-Issues/Augmentative-and-Alternative-Communication/"
    organisation: "American Speech-Language-Hearing Association"
    note: "Использовано для описания PECS как одного из подходов AAC и структуры протокола."
    accessedAt: 2026-09-18
  - title: "Effectiveness of the Picture Exchange Communication System (PECS) on communication and speech for children with autism spectrum disorders: a meta-analysis"
    url: "https://pubmed.ncbi.nlm.nih.gov/20181849/"
    organisation: "PubMed"
    publicationYear: 2010
    note: "Ранний метаанализ: небольшие или умеренные улучшения коммуникации, ограниченный эффект на речь и ограничения по сохранению и переносу навыка."
    accessedAt: 2026-09-18
  - title: "The effectiveness of Picture Exchange Communication System (PECS) training for teachers of children with autism: a pragmatic, group randomised controlled trial"
    url: "https://pubmed.ncbi.nlm.nih.gov/17501728/"
    organisation: "PubMed"
    publicationYear: 2007
    note: "Кластерное рандомизированное исследование: использование PECS и коммуникативные инициативы увеличились, но значимого роста частоты речи или стандартизированных языковых показателей не было."
    accessedAt: 2026-09-18
  - title: "A communication-based intervention for nonverbal children with autism: what changes? Who benefits?"
    url: "https://pubmed.ncbi.nlm.nih.gov/21787048/"
    organisation: "PubMed"
    publicationYear: 2011
    note: "Вторичный анализ: отмечены улучшения спонтанной коммуникации, особенно просьб о предметах или действиях."
    accessedAt: 2026-09-18
  - title: "Communication interventions for autism spectrum disorder in minimally verbal children"
    url: "https://www.cochrane.org/evidence/CD012324_are-communication-interventions-effective-minimally-verbal-children-autism-spectrum-disorder"
    organisation: "Cochrane"
    note: "Более старый обзор с поиском до 2017 года; на тот момент уверенность в данных рандомизированных исследований PECS была очень низкой."
    accessedAt: 2026-09-18
  - title: "Efficacy of the Picture Exchange Communication System for children with autism in Mainland China: A systematic review and meta-analysis of randomized controlled trials"
    url: "https://pubmed.ncbi.nlm.nih.gov/41442835/"
    organisation: "PubMed"
    publicationYear: 2026
    note: "37 рандомизированных исследований, 34 включены в количественный синтез, всего 2343 участника; сообщён крупный объединённый эффект для коммуникации и отсутствие значимого объединённого эффекта для развития языка."
    accessedAt: 2026-09-18
  - title: "A systematic review of acquisition and mastery of skills taught using the Picture Exchange Communication System"
    url: "https://pubmed.ncbi.nlm.nih.gov/38300727/"
    organisation: "PubMed"
    publicationYear: 2024
    note: "Использовано для ограничений, связанных с освоением фаз и неоднородностью критериев."
    accessedAt: 2026-09-18
  - title: "Facilitators and barriers to the implementation of the picture exchange communication system (PECS): a systematic review"
    url: "https://www.frontiersin.org/journals/education/articles/10.3389/feduc.2024.1206410/full"
    organisation: "Frontiers in Education"
    publicationYear: 2024
    note: "Использовано для ограничений, связанных с качеством реализации, обучением, контекстом и супервизией."
    accessedAt: 2026-09-18
---

[← Коммуникация и AAC](/kz/ru/parents/kommunikatsiya-i-aac/)

[← AAC: альтернативная и дополнительная коммуникация](/kz/ru/parents/aac/)

PECS (Picture Exchange Communication System - система обмена изображениями для коммуникации) - это конкретный структурированный AAC-подход, в котором человека обучают инициировать коммуникацию с помощью обмена изображениями.

Важно сразу различать:

**PECS является одним из вариантов AAC.**

Но:

**PECS не является синонимом всей AAC.**

AAC включает гораздо больше способов коммуникации: жесты, коммуникационные доски, приложения, устройства с синтезом речи, письмо и другие варианты.

PECS также не является обязательной первой ступенью, которую человек должен пройти перед использованием высокотехнологичной AAC.

## Как работает PECS

PECS представляет собой стандартизированный протокол с шестью последовательными фазами обучения.

Сначала человек учится обменивать одно изображение на желаемый объект или действие, затем увеличивается самостоятельность и расстояние до партнёра по коммуникации, появляется выбор между изображениями, построение простой фразы, ответы на вопрос и позднее комментарии.

То есть PECS - не просто использование карточек.

## PECS и обычные карточки - не одно и то же

Ребёнок может использовать:

- визуальное расписание;
- карточку «перерыв»;
- доску выбора;
- изображения предметов;
- визуальные инструкции.

Это ещё не означает, что используется PECS.

PECS - конкретный протокол обучения.

AAC на основе изображений в целом значительно шире.

## Кому PECS может быть полезен

PECS чаще всего изучался у детей с аутизмом и другими нарушениями развития, особенно когда функциональная устная речь ограничена.

Он может быть одним из вариантов, если:

- человеку подходит коммуникация через изображения;
- важной целью является самостоятельная инициация коммуникации;
- текущего способа коммуникации недостаточно;
- обмен изображениями удобен в реальных ситуациях.

Но диагноз, возраст или отсутствие речи сами по себе не означают, что PECS обязательно будет лучшим вариантом.

Выбор между PECS и другими AAC-системами должен происходить после оценки коммуникационных потребностей.

## Что показывали более ранние исследования

Метаанализ 2010 года включил 8 экспериментов с единичными случаями и 3 групповых исследований.

Авторы обнаружили небольшие или умеренные улучшения коммуникационных результатов после PECS, но улучшения устной речи были небольшими или отрицательными.

Также были отмечены проблемы с сохранением навыка и переносом навыка.

Это была относительно небольшая доказательная база, поэтому тогда PECS характеризовался скорее как перспективное вмешательство, чем как окончательно установленный стандарт.

## Что показало крупное школьное рандомизированное контролируемое исследование

В Великобритании было проведено кластерное рандомизированное контролируемое исследование с участием 84 детей.

Учителя проходили обучение PECS и получали консультации специалистов в течение пяти месяцев.

После вмешательства увеличились:

- коммуникативные инициативы;
- использование PECS.

Но исследование не обнаружило значимого увеличения частоты речи, улучшения стандартизированных языковых показателей или показателей коммуникации ADOS.

После прекращения активного вмешательства эффекты использования PECS и коммуникативных инициатив не сохранялись.

Это важный пример того, почему нельзя говорить просто:

**«PECS улучшает коммуникацию».**

Нужно уточнять, какой именно коммуникационный результат.

## Что показал последующий анализ

Дополнительный анализ того же рандомизированного исследования показал увеличение самостоятельной коммуникации после обучения PECS.

Особенно заметно увеличилось число самостоятельных просьб о предметах.

При этом число просьб с социальной целью статистически значимо не увеличилось.

Часть результатов также зависела от исходных характеристик ребёнка.

Поэтому наиболее сильные данные исторически относились прежде всего к инструментальной функциональной коммуникации, а не ко всем возможным видам общения.

## Что говорил Cochrane

Обзор Cochrane по коммуникационным вмешательствам у аутичных детей с минимальной устной речью включил только одно подходящее рандомизированное исследование PECS.

Авторы оценили уверенность в доказательствах как **очень низкую** из-за:

- риска систематической ошибки;
- небольших выборок;
- широких доверительных интервалов;
- фактически одного рандомизированного исследования для PECS.

Важно помнить, что поиск в этом обзоре заканчивался в 2017 году, поэтому он не включает более новую литературу.

## Новые данные 2026 года

В январе 2026 года опубликован крупный систематический обзор и метаанализ PECS у детей с аутизмом в материковом Китае.

В него вошли 37 рандомизированных контролируемых исследований, из которых 34 вошли в метаанализ, всего 2343 участника.

Авторы сообщили крупный объединённый эффект на коммуникационные навыки:

**SMD 0.94, 95% CI 0.67-1.21.**

Также были заявлены улучшения ряда дополнительных результатов.

При этом статистически значимого эффекта на **развитие языка** не обнаружили:

**SMD 0.44, 95% ДИ от -0.53 до 1.41.**

Это существенно расширяет современную доказательную базу PECS.

## Почему новый метаанализ не означает «PECS доказан для всего»

Этот результат важен, но его нужно интерпретировать осторожно.

Все включённые исследования были проведены в материковом Китае.

Это означает, что:

- системы помощи могли отличаться;
- условия могли отличаться;
- подготовка специалистов могла отличаться;
- условия сравнения могли отличаться;
- культурный и образовательный контекст отличается от Казахстана и других стран.

Кроме того, некоторые объединённые эффекты по дополнительным результатам в метаанализе были очень крупными, поэтому особенно важны независимое воспроизведение результатов и оценка отдельных исследований.

Поэтому разумный вывод сегодня:

**PECS имеет подтверждающие данные для некоторых коммуникационных результатов, но нельзя переносить этот вывод на все возможные результаты или на каждого ребёнка.**

## PECS и развитие речи

PECS иногда рекламируют как метод, который должен «запустить речь».

Такая формулировка слишком сильная.

Метаанализ 2010 года не показал убедительных улучшений речи.

Школьное рандомизированное исследование не обнаружило улучшения частоты речи или стандартизированных языковых показателей.

Метаанализ 2026 года не выявил статистически значимого объединённого эффекта на развитие языка.

Это не означает, что у отдельных детей речь никогда не меняется после PECS.

Но обещание:

**«PECS запускает речь»**

не соответствует текущей доказательной базе.

Основная цель PECS - функциональная коммуникация.

## Нужно ли начинать именно с PECS

Нет.

PECS является одним из вариантов AAC.

Другими вариантами могут быть:

- коммуникационная доска;
- низкотехнологичная AAC;
- жесты;
- устройство с синтезом речи;
- AAC на планшете;
- комбинированная мультимодальная коммуникация.

Современный подход к AAC не требует проходить PECS перед использованием электронной AAC.

Поэтому правильный вопрос:

**Какой способ коммуникации лучше соответствует потребностям этого человека?**

а не:

**«Прошёл ли он PECS перед планшетом?»**

[Подробнее об AAC](/kz/ru/parents/aac/)

## Нужно ли пройти все шесть фаз

Не обязательно считать прохождение всех фаз главным результатом.

Систематический обзор 2024 года изучал освоение навыков на разных этапах PECS и показал, что исследования существенно различались в определении критерия освоения, а далеко не все участники проходили все шесть фаз.

Поэтому:

**завершение фазы VI (Phase VI) не равно автоматически успешной коммуникации.**

Важнее проверить, помогает ли система человеку в реальной жизни.

## Качество реализации имеет значение

Систематический обзор 2024 года изучал факторы, которые мешают или помогают внедрять PECS, в 49 исследованиях.

Авторы обнаружили, что ключевые детали реализации часто сообщались недостаточно.

В литературе встречались такие возможные факторы, как:

- контекст и условия среды;
- обучение и консультации;
- супервизия (supervision);
- соответствие протоколу;
- соблюдение протокола (adherence to protocol).

Но имеющихся данных оказалось недостаточно, чтобы уверенно определить, какие именно факторы надёжно предсказывают лучший результат.

То есть формула:

**«главное строго соблюдать протокол и результат будет»**

тоже слишком упрощённая.

## Возможные ограничения PECS

PECS обычно не относится к физически рискованным вмешательствам.

Но возможны другие проблемы.

### Коммуникация остаётся только на уровне просьб

Если ребёнок умеет только:

**«хочу сок»**

или

**«хочу игрушку»**,

это ещё не полноценная коммуникационная система.

Нужны также возможности:

- отказа;
- комментария;
- сообщения о боли;
- вопросов;
- рассказа;
- общения со сверстниками;
- самостоятельного отстаивания своих потребностей и прав (self-advocacy).

### Зависимость от подсказок

Если ребёнок использует PECS только после:

**«дай карточку»**

или физической подсказки, самостоятельная коммуникация остаётся ограниченной.

### Нет переноса навыка

Навык может существовать только:

- в кабинете;
- с одним специалистом;
- с одним набором карточек.

Нужно проверять его в обычной жизни.

### PECS продолжают использовать, хотя другой AAC уже удобнее

Иногда человек мог бы быстрее и свободнее общаться через устройство с синтезом речи (speech-generating device, SGD), текст или другую систему.

Не стоит сохранять PECS только потому, что в него уже вложено много времени.

## Как понять, помогает ли PECS

Не оценивайте PECS только по:

- количеству обменов изображениями;
- количеству выполненных учебных попыток;
- номеру освоенной фазы.

Лучше определить конкретный функциональный результат.

Например:

**самостоятельно попросить помощь**

**отказаться**

**попросить перерыв**

**инициировать общение**

**сделать комментарий**

После этого отслеживайте:

- исходный уровень;
- число самостоятельных коммуникативных инициатив;
- число подсказок;
- количество партнёров по коммуникации;
- использование дома;
- использование в школе;
- разнообразие сообщений;
- выраженный дискомфорт;
- удобство системы.

Главный вопрос:

**становится ли человеку легче самостоятельно сообщать то, что он действительно хочет сказать?**

[Как понять, помогает ли выбранная помощь](/kz/ru/parents/kak-ponyat-pomogaet-li-podderzhka/)

## Когда стоит пересмотреть PECS

Пересмотреть систему полезно, если:

- коммуникация остаётся только просьбами;
- человеку постоянно нужны подсказки;
- PECS не используется вне занятий;
- обмен карточками физически неудобен;
- словарь плохо расширяется;
- ребёнок избегает коммуникационной книги;
- другой AAC быстрее или удобнее;
- изменились двигательные возможности, зрение или навыки чтения и письма;
- система ограничивает самостоятельность.

Изменение AAC-системы не означает неудачу.

Цель - не сохранить конкретный протокол.

Цель - сохранить эффективную коммуникацию.

## Что спросить специалиста

1. Почему предлагается именно PECS?
2. Какие другие варианты AAC рассматривались?
3. Какова конкретная коммуникационная цель?
4. Как будет измеряться самостоятельная инициация коммуникации?
5. Как будут уменьшаться подсказки?
6. Как проверят перенос навыка?
7. Как ребёнок сможет отказаться?
8. Как будут добавляться комментарии и другие функции кроме просьбы?
9. Когда PECS сравнят с другими вариантами AAC?
10. Что будет основанием сменить систему?
11. Какая базовая профессиональная квалификация у специалиста?
12. Является ли сертификат PECS дополнительным обучением или основной квалификацией?

## Сертификат PECS и квалификация специалиста

Прохождение обучения PECS подтверждает обучение конкретному протоколу.

Но само по себе оно не означает, что человек автоматически является:

- логопедом;
- психологом;
- врачом;
- педагогом;
- эрготерапевтом (occupational therapist);
- другим регулируемым специалистом.

Это отдельные вопросы.

## Что важно знать в Казахстане

В Казахстане PECS может использоваться в:

- образовательных организациях;
- коррекционных центрах;
- медицинских учреждениях;
- психологических службах;
- частной практике.

Поэтому при выборе специалиста отдельно проверяйте:

**его базовую профессиональную квалификацию**

и

**обучение PECS**.

Сертификат PECS:

**не заменяет базовую профессиональную квалификацию**

и

**не является доказательством эффективности услуги у конкретного ребёнка.**

## Короткий итог

PECS - это один конкретный подход AAC.

Современные данные поддерживают его способность улучшать некоторые виды функциональной коммуникации, особенно инициативы в коммуникации и просьбы.

При этом:

- PECS не равен всей AAC;
- PECS не обязателен перед электронной AAC;
- убедительного гарантированного эффекта на речь нет;
- перенос и сохранение навыка нужно проверять отдельно;
- прохождение всех фаз не является главным результатом;
- новый крупный метаанализ 2026 года сообщает положительные результаты для коммуникации, но переносить эти выводы за пределы изученного контекста нужно осторожно.

## Навигация по маршруту

- Предыдущий материал: [«Обучение партнёров по коммуникации»](/kz/ru/parents/communication-partner-training/)
- Следующий материал: [«Обучение функциональной коммуникации: когда поведение заменяет сообщение»](/kz/ru/parents/functional-communication-training/)
- Также по теме: [«AAC: альтернативная и дополнительная коммуникация»](/kz/ru/parents/aac/)
