...
> **Сначала понять, что происходит и какую проблему нужно решить. Потом выбирать вмешательство.**
...
Если поведение новое, резко усилилось или опасно, сначала может быть нужна медицинская или мультидисциплинарная оценка.

## 1. Функциональная оценка поведения (Functional Behavior Assessment, FBA)
...
## 2. Обучение функциональной коммуникации (Functional Communication Training, FCT)

Если оценка показывает, что поведение помогает человеку сообщать:
...
## 3. Изменение условий до поведения (Antecedent-Based Interventions)
...
Постепенное уменьшение подсказок (prompt fading) помогает увеличить самостоятельность.
...
## 7. Разбиение навыка на шаги (Task Analysis)
...
## 8. Обучение через показ и видеомоделирование
...
## 9. Самоуправление (Self-Management)

Самоуправление помогает переносить часть контроля от взрослого к самому человеку.
...
если они не причиняют значимого вреда.

Цель вмешательства должна быть связана с:
...
3. Зафиксировать исходный уровень.
4. Провести функциональную оценку, если она нужна.
5. Выбрать конкретную функциональную цель.
6. Выбрать подход, соответствующий этой цели.
7. Заранее определить показатели результата.
8. Проверить перенос навыка в реальную жизнь.
9. Пересмотреть план, если значимой пользы нет.
...
2. Проверялись ли медицинские и сенсорные причины?
3. Проводилась ли функциональная оценка?
4. Как называется вмешательство по-русски?
