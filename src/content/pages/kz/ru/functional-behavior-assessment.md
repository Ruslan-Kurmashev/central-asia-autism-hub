...
updatedAt: "2026-09-20T14:01:00.617Z"
...
Функциональная оценка поведения (Functional Behavior Assessment, FBA) - это процесс оценки, который помогает понять, какие факторы могут провоцировать или поддерживать сложное поведение.
...
Хорошая функциональная оценка должна собрать данные и проверить, какие медицинские, коммуникационные, сенсорные, социальные факторы и особенности среды связаны с поведением.

## Когда функциональная оценка может быть полезна
...
- регулярно приводит к сильному дискомфорту
...
Например, повторяющееся движение или другое аутичное поведение, которое не причиняет значимого вреда, не нужно автоматически пытаться уменьшать.
...
- ситуации, в которых поведение появляется
- ситуации, в которых поведение не появляется
- возможные triggers
...
До вмешательства полезно знать, что происходит сейчас:

- сколько раз поведение возникает
- сколько длится
- в каких ситуациях
...
**Предшествующее событие - поведение - последствие (ABC).**
...
Оценка может включать интервью с родителями, педагогами, ухаживающими взрослыми и самим человеком, если это возможно.
...
## FBA и экспериментальный функциональный анализ - не одно и то же

### Функциональная оценка поведения

Это широкий процесс оценки.
...
Если поведение включает тяжёлое самоповреждение, агрессию, опасные попытки убежать или другой непосредственный риск для безопасности, родителям не следует самостоятельно проводить экспериментальную проверку функции поведения.

## Функция поведения не всегда одна

Одинаковое поведение может происходить по разным причинам.
...
Если оценка показывает такую закономерность, одним из вариантов может стать:
...
Если смотреть только на поведение ребёнка, можно пропустить проблему в окружающей среде.
...
Оценка должна привести к конкретному плану.
...
- другое вмешательство, основанное на функции поведения.
...
- уменьшился ли выраженный дискомфорт
...
Если поведение остаётся серьёзной проблемой после попыток устранить возможные причины, может понадобиться мультидисциплинарная оценка.

Это может включать специалистов по физическому и психическому здоровью, коммуникации, образованию, среде и поддержке поведения.

## Что спросить специалиста

1. Как именно определено целевое поведение?
2. Какие медицинские причины проверялись?
...
11. Как оценка будет связана с выбранным вмешательством?
...
Нужно отдельно проверять базовую профессию, образование, профессиональную роль и границы компетенции.

## Короткий итог

Функциональная оценка поведения - это не терапия и не способ быстро прикрепить к поведению ярлык.

Хорошая FBA должна:

- проверить health and environmental factors
- конкретно определить поведение
...
- привести к плану, основанному на функции поведения
- измерять значимые результаты.
